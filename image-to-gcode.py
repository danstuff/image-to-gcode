...
def make_tool_shape(f, wdia, resp, is_offset= False, tool_type=0, wdia2=-9999, degr=60, units=0):
    ...
    #n = numarray.array([[plus_inf] * dia] * dia, type=numpy.float32)
    n = numarray.array([[plus_inf] * dia] * dia, numpy.float32)
    hdia = dia / 2.
    #hdia = int(hdia)
    l = []
...
def optim_tool_shape(n,hig,offset,tolerance):
    ...
    if lineLF > 0:
        ...
        if line2 < dia:
            dia2 = line2 - lineLF
            n2 = numarray.array([[plus_inf] * dia2] * dia2, numpy.float32)
...
class Converter:
    def __init__(self,
        ...
        if self.roughing_minus_finishing:
            ...
            if self.row_mill:
                self.map_tool2 = numarray.zeros((w, h), numpy.float32) - plus_inf

                # map_rmf>=0 - True;  map_rmf<0 - False
                self.map_rmf = numarray.zeros((w, h), numpy.float32) - plus_inf
            else:
                self.map_tool2 = numarray.zeros((h, w), numpy.float32) - plus_inf

                # map_rmf>=0 - True;  map_rmf<0 - False
                self.map_rmf = numarray.zeros((h, w), numpy.float32) - plus_inf
            self.map_tool2 = numarray.zeros((w, h), numpy.float32) - plus_inf
    ...
    def one_pass(self):
        ...
        if prn_detail > 0:
            ...
            if printToFile:
                print("Layer {0} layer depth {1} has started, please wait...".format(self.layer,self.layer_depth), file=originalStdout)
    ...
    def make_offset(self,offset,image):
        if offset > epsilon16:
            ...
            w1 = w + tw
            h1 = h + th
            nim1 = numarray.zeros((w1, h1), numpy.float32) + image.min()
            nim1[tw//2:tw//2+w, th//2:th//2+h] = image
            image = numarray.zeros((w,h), numpy.float32)
            for j in range(0, w):
                progress(j,w)
                for i in range(0, h):
    ...
    def convert(self):
        ...
        if prn_detail > -1 and self.MaxBackground_up <= self.MaxBackground_down:
                ...
                print("Error(!): Max background border down[{0}] >= Max background border up[{1}]! G-code can not be formed.".format(self.MaxBackground_down, self.MaxBackground_up))
    ...
    def get_rmf_map_tool(self,offset_image,tool_roughing,previous_offset,pixelstep_roughing):
        ...
        map_tool1 = {}  #numarray.zeros((self.w, self.h), numpy.float32) - self.image.min()
        ...
        for j in lines:
            ...
            if self.row_mill:
                ...
                if len(scan) != 0:
                    ...
                    if prn_detail > -1: print("Error(0) g-code not make L: {0} [{1}, {2}] !!!".format(clayer,P_f,P_l))
            else:
                #Cols
                ...
                if len(scan) != 0:
                    ...
                    if prn_detail > -1: print("( ",clayer,", Error len(scan) == 0! g-code not make from ",P_f," to ",P_l," !!!)")
            self.g.flush()
    ...
    def mill_objectiv(self, convert_scan, primary):
        ...
        if self.background_border > .0 and self.roughing_minus_finishing:
            ...
            #   in RMF we need make G-code in some places with the background,
            #       but not everywhere where there is background!
            #  Therefore, we first determine where to make g-code, and where do not make g-code.
            #  Therefore, in RMF mode - param Max_BG_len is works in a special way.
            #
            #For other types of creating g-code - "BG filtering" integrated in proc-s: get_entry_pixels_go_XXX, not_background and pixel_use
...
def main():
    ...
    w, h = im.size
    try:
    	nim = numarray.fromstring(im.tobytes(),numpy.uint8,-1).astype(numpy.float32)
    except AttributeError:
        nim = numarray.fromstring(im.tostring(),numpy.uint8,-1).astype(numpy.float32)
    nim = nim.reshape(w,h)
    ...
    if generateGcode:
        ...
        if options['expand']:
            ...
            w1 = w + 2*tw
            h1 = h + 2*th
            nim1 = numarray.zeros((w1, h1), numpy.float32) + pixel
